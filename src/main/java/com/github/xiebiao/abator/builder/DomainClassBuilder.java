...
public class DomainClassBuilder extends ClassBuilder {
    ...
    protected void buildField() throws Exception {
        ...
        if (columns == null) {
            ...
            for (Column c : columns) {
                LOG.debug("Table name:" + this.table.getName() + " column:" + c.getName());
    ...
    protected void buildSetterGetter() throws Exception {
        ...
        for (Column c : columns) {
            ...
            sb.append("\n");
            sb.append(
                    tab + "public void set" + field.substring(0, 1).toUpperCase()
                            + field.substring(1, field.length()) + "(" + getJavaDataType(c) + " " + _field + ")")
    ...
    protected void buildToString() {
        ...
        for (int i = 0; i < size - 1; i++) {
            sb.append(tab + tab + "sb.append(\"\\\"" + fields.get(i) + "\\\":\\\"\"+this."
                    + fields.get(i) + "+\"\\\"\");\n");
            if (i < (size - 2)) {
                sb.append(tab + tab + "sb.append(\",\");\n");
            }
